...
const NotificationsPage: React.FC = () => {
    ...
    return (
        <div className="flex h-[calc(100vh-81px)] bg-white overflow-hidden">
            <Navbar />
            <div className="flex-1 flex">
                {/* Left Panel: Chat List */}
                <div
                    className={`w-[280px] flex flex-col border-r border-gray-200 ${selectedToId !== null && window.innerWidth < 768 ? 'hidden' : 'flex'}`}>
                        ...
                        {chatList.length > 0 ? (
                            <ul>
                                {chatList.map(({ chatUser, unRead }) => (
                                    <li
                                        key={chatUser.id}
                                        className={`flex items-center py-[12px] px-[12px] border-b border-gray-200 hover:bg-gray-50 cursor-pointer ${selectedToId === chatUser.id.toString() ? 'bg-light-bg-theme/40 border-r-4 border-blue-500' : ''}`}
                                        onClick={() => handleChatClick(chatUser.id)}>
                                        <img
                                            src={
                                                chatUser.avatar || defaultAvatar
                                            }
                                            alt={chatUser.name}
